...
async function iniciarCadastro(){
  ...
  try{
    ...
    if(login.verificationRequired){
      ...
      const codigo=(await titanPrompt('Mandamos um código de 6 dígitos pro e-mail '+(login.emailHint||'cadastrado')+'. Não chegou em 1 minuto? Verifique a caixa de spam ou lixo eletrônico. Digite o código abaixo para confirmar este acesso.','','Confirme este acesso')||'').trim();
...
async function solicitarTrocaEmailMaster(){
  ...
  try{
    ...
    const codigo=(await titanPrompt('Mandamos um código de 6 dígitos pro e-mail '+(resp.emailHint||'informado')+'. Não chegou em 1 minuto? Verifique a caixa de spam ou lixo eletrônico. Digite o código abaixo para confirmar a troca.','','Confirme o novo e-mail')||'').trim();
...
// A coluna "Carteira" da Gestão por CNPJ move a EMPRESA para a carteira do
// parceiro (companies.partner_id) — é isso que faz o CNPJ aparecer no Portal
// do Parceiro. Até 22/08/2026 ela chamava a rota de user_partners, que só
// marca quem opera o CNPJ por dentro: o Master vinculava, a tela confirmava, e
// o parceiro continuava sem enxergar empresa nenhuma.
async function vincularEmpresaParceiro(companyId,partnerId){
  try{
    const data=await api('/api/master/companies/'+companyId+'/partner',{method:'PUT',body:JSON.stringify({partnerId:partnerId||null})});
    const empresa=masterData?.companies.find(item=>item.id===companyId);
    if(empresa)empresa.partner_id=data.partner_id;
    renderMasterClients();
  }catch(error){alert(error.message);await carregarMaster()}
}
